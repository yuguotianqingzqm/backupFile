		...
		methods: {
			...
			totalPrice1:function(){
				...
				for(var i=0;i<this.goods.length;i++){
					this.totalPrice+=Number(this.goods[i].foodPrice)*Number(this.goods[i].number);
				}
			},
			//添加购物车
			add:function(index){
				<c:forEach items="${foods}" var="foods" varStatus="s">
				if(index==${s.index}){
					this.goods.push({foodId:"${pageContext.request.getSession(false).getAttribute("user")[0].userId}",foodName:"${foods.foodName}",foodPrice:"${foods.foodPrice}",number:"1",foodImage:"${pageContext.request.contextPath}/upload/${foods.foodImage}"});
				}
				//计算总价格
				this.totalPrice1();
				</c:forEach>
